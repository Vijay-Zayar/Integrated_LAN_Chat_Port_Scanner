...
def runServer(right_frame):
    ...
    # stop_btn = ttk.Button(inner_frame, text="Stop", bootstyle="WARNING", width=10, command=lambda: menus.chooseClientServer(right_frame))
        ...
        try:
            if server_thread is None or not server_thread.is_alive():
                ...
                try:
                    ...
                    message_box.show_info(f"Running the server on {HOST} : {PORT}","Server Started", parent=parent_frame)
            ...
            # initial_server = True
            message_box.show_error("Unable to start the server !", "Connection Error", parent=parent_frame)
    ...
    def stop_server(parent_frame): 
        ...
        if len(active_clients) == 0 and server_socket and server_thread.is_alive():
            server_status = False
            ...
            try:
                ...
                if message == 'LEAVE':
                    # Remove the leaving client from the list of active clients
                        ...
                        leave_message = "SERVER~" + f"{username} has left the chat"
                        send_messages_to_all(leave_message)

                        try: 
                            # Close the client socket
                            print("Before closing client socket")
                            client.close()
                            print("After closing client socket")
                        except:
